...
def test_fetch_node_success(empty_triage_state):
    ...
    with (
        patch("nodes.fetch_node.get_incident") as mock_get_incident,
...
def test_fetch_node_alerts_failure(empty_triage_state):
    ...
    with (
        patch("nodes.fetch_node.get_incident") as mock_get_incident,
        patch(
            "nodes.fetch_node.list_incident_alerts",
            side_effect=Exception("Alerts down"),
        ),
    ):

        mock_get_incident.return_value = {"properties": {"title": "Suspicious Login"}}
